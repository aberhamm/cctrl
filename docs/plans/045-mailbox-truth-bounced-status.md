...
- [ ] New terminal status `bounced` — defined as **addressee unresolvable at deliver time** (the store cannot prove the addressee ever existed; only `--allow-unknown` sends record `unknown_peer:true`): when a deliver pass finds a queued message whose addressee no longer resolves to any live or manual peer, the row transitions `queued → bounced` with a history event carrying the reason (`unknown-peer`) and timestamp. All three `unknown-peer` sites bounce: the `--all` loop in the deliver command, `_peer_orchestrate_recipients_locked`, and targeted (non-`--all`) deliver — which today returns `unknown-peer` (exit 66) without producing a delivery result or rewriting rows and needs its own bounce handling — otherwise check/status disagree depending on entrypoint. Targeted deliver's post-bounce contract: it performs the bounce transition, emits a `bounced` result row (`--json` included), and keeps its non-zero exit. Distinct from `blocked` (addressee replaced — plan 032), which keeps its meaning. **Bounce requires trustworthy discovery:** a row transitions to `bounced` ONLY when the peers document was fully built (`derived_skipped == false`). When discovery was skipped — tmux missing, `_session_list` erroring, invalid JSON (`_peer_derived_json` sets `skipped:true, peers:[]`) — every derived peer looks unknown, and bouncing then would terminally poison all live peers' queued mail in one bad pass; those passes keep today's retryable `unknown-peer` skip. Test: a discovery-skipped deliver pass leaves rows `queued`.
...
2. Add the `bounced` transition at all three `unknown-peer` sites (deliver `--all` loop, `_peer_orchestrate_recipients_locked`, targeted deliver — the last needs its own bounce handling since it currently rewrites nothing), each under the mailbox lock with a re-resolve of the addressee immediately before the write (TOCTOU). The pre-bounce re-resolve MUST bypass the plan-027 memoized `_PEER_ALL_JSON_CACHE` — on a cache-primed caller `_peer_resolve_json` silently serves the pre-lock document, making the re-resolve a no-op. Gate every bounce on `derived_skipped == false` (see Requirements); with history event + reason. Tests: a bounced row stays bounced after the name is re-registered; a discovery-skipped pass leaves rows `queued`.
