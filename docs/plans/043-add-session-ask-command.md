...
- [ ] Correlation is nonce-based: the command locates the transcript **user** turn containing the nonce (this doubles as delivery confirmation), then waits for the **next assistant turn after that user turn** to complete AND the session's bridge status to return to idle, and extracts exactly that assistant text. The nonce is matched against user-turn TEXT content only — transcript lines with role `user` include tool_result entries, and those must never match.
...
5. Tests: happy path, noise interleave, `not-echoed`, `no-answer`, `transcript-replaced`, non-Claude refusal, modal refusal.
